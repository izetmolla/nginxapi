...
func NewCertificatesStorage(ctx *SetupConfig) (*CertificatesStorage, error) {
	pfxFormat := ctx.PfxFormat
	...
	return &CertificatesStorage{
		rootPath:    filepath.Join(ctx.Path, baseCertificatesFolderName),
		archivePath: filepath.Join(ctx.Path, baseArchivesFolderName),
		pem:         ctx.Pem,
		pfx:         ctx.Pfx,
		pfxPassword: ctx.PfxPass,
		pfxFormat:   pfxFormat,
...
func (s *CertificatesStorage) CreateRootFolder() error {
	...
	if err != nil {
		return fmt.Errorf("could not check/create path: %v", err)
	}
	return nil
}
...
func (s *CertificatesStorage) ExistsFile(domain, extension string) bool {
	...
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		...
		fmt.Println("error on ExistsFile: ", err)
		return false
	}
	return true
}
